run.sim = function(prefix="",true_k=c(2:7),fold_change=c(0.5,1,2),num_disc=c(.1,.2),g=c(300,1000),n=c(50,100,200),
                   ...
                   ncores=10,prefilter="F",pval_thresh=0.4){
  ...
  for(i in 1:length(true_k)){for(j in 1:length(fold_change)){for(k in 1:length(num_disc)){for(l in 1:length(g)){for(m in 1:length(n)){
      ...
      cmd[2] = sprintf("X = sim.EM(true.K = %d, fold.change =%f, num.disc = %f, g = %d,n = %d,
                       distrib='%s', method='%s', prefilter=%s, pval_thresh=%f,
                       disp='%s', fixed_parms=%s, fixed_coef=%f, fixed_phi=%f,
                       ncores=%d)\n",
                       true_k[i],fold_change[j],num_disc[k],g[l],n[m],distrib,method,prefilter,pval_thresh,disp,fixed_parms,fixed_coef,fixed_phi,ncores)
      if(fixed_parms=="F"){
        out = sprintf("/netscr/deelim/run_sim_%s_%s_%s_%s_%d_%f_%f_%d_%d_filt%s_%f",
                    prefix,distrib,method,disp,true_k[i],fold_change[j],num_disc[k],g[l],n[m],prefilter,pval_thresh)
      } else{
        out = sprintf("/netscr/deelim/run_sim_%s_%s_%s_%s_%d_%f_%f_%d_%d_fixed_%f_%f_filt%s_%f",
                      prefix,distrib,method,disp,true_k[i],fold_change[j],num_disc[k],g[l],n[m],fixed_coef,fixed_phi,prefilter,pval_thresh)
      }
...
collect.sim = function(prefix="",true_k=c(2:7),fold_change=c(0.5,1,2),num_disc=c(.1,.2),g=c(300,1000),n=c(50,100,200),
                       distrib="nb",method="EM",disp="gene",fixed_parms="F", fixed_coef=6.5,fixed_phi=0.35,prefilter="F",pval_thresh=0.4){
  ...
  if(fixed_parms=="F"){
    final_results = sprintf("final_table_%s_%s_%s_filt%s_%f",distrib,method,disp,prefilter,pval_thresh)
  } else{
    final_results = sprintf("final_table_%s_%s_%s_fixed_%f_%f_filt%s_%f",distrib,method,disp,fixed_coef,fixed_phi,preilter,pval_thresh)
  }
