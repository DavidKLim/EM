...
% The transformation is applied for each $k=1,...K$, so the response is multiplied by the number of clusters: $n*K$ transformed responses. $\hat{\mu_k}$ and $\hat{\eta_k}$ are iteratively updated within the IRLS algorithm until convergence.
...
\begin{center}
<<echo=FALSE,fig=TRUE>>=
load("Project1/list_BIC_n100_g865_K3.Rout")
...
(WILL ADD: GENE-WISE vs CLUSTER-WISE DISPERSION PARAMETERS. Concern: ie. 10\% disc, LFC=1, n=100, g=1000, K=2 not choosing correct order. noise? at 20\%, correct order chosen. also at LFC=0.5, never finding correct order. More inaccurate as K increases. Cl-wise gave better results (previously))

We simulated 100 datasets per condition, and spanned an extensive set of conditions. Variable conditions included log fold change (LFC), number of clusters (K), number of samples (n), number of genes (g), and proportion of discriminatory genes (pdisc). (ALSO INCLUDE VARYING NOISE?) In order to portray an accurate relationship between expression level and noise, we simulated genes based on the coefficient and dispersion estimates of the genes from the TCGA breast cancer dataset, then (for discriminatory genes) we imposed the LFC across the simulated clusters on the mean.

<<xtable1, results=tex, echo=FALSE>>==
library(xtable)
load("/Users/deelim/Documents/Research/Simulations/final_table_nb.out")
colnames(tab) = c("n","g","lfc","true.K","true.pdisc","K","pdisc","lambda2","tau","ARI","sens","false.pos")
tab1 = xtable(tab[,c(6,3,5,7,10:12)],
              digits=2,
              table.placement="!h",
              caption="Varying log fold change (lfc) and proportion of discriminatory genes (true.pdisc) for K=3, n=50, g=1000",
              label = "tab:1")      # K=3, n=50, g=1000
print(tab1,include.rownames=F)
@

(NOTE: table is inaccurate. For incorrectly selected order, ARI here is based on input correct order. Also, this table was based on cluster-specific dispersion parameters) We note from \cref{tab:1} that the order is incorrectly estimated in the case of a small proportion of discriminatory genes. Also, as the LFC is increased, we see a general increase in the ARI, indicating greater clustering accuracy when the differential between clusters is larger.

Another useful feature of our algorithm is the ability to uncover which genes are discriminatory across clusters. Our method seems to uncover the true proportion of discriminatory genes with perfect sensitivity, and very low false positive rates (FPR). We expect to see performance decrease with a smaller LFC, since the algorithm will have a harder time distinguishing the clusters. However, we see the FPR is quite low even in the relatively small LFC of $0.03$. 

(WILL INCLUDE MORE TABLES VARYING n, g, and K)
...
# Silhouette
load("/Users/deelim/Documents/Research/Sweave/Project1/BRCA_dist.out")
...
tab2 = data.frame(rbind(round(EMclust,3),round(iclust,3)))
colnames(tab2) = c("K","ARI","Silhouette")
rownames(tab2) = c("EM","iCluster+")

tab2 = xtable(tab2,caption="Comparing iCluster+ with EM method. True (annotated) order is 4. Values of ARI closer to 1 indicate better clustering results with respect to the true (annotated) clusters. Silhouette values cluster to 1 indicate better clustering results based on dissimilarity matrix on normalized counts.",label="tab:2")
print(tab2)
...
It it known that the PAM50 genes are genes of significant interest for breast cancer. Thus, we performed further analysis on just the PAM50 genes. \cref{fig:2} is a heatmap of the known PAM50 genes, ordered by the clustering results of our CEM, and using cluster-specific dispersion parameters. (NOTE TO SELF: with initial Tau = 2. need to be re-run and made bigger to see gene labels)

\begin{figure}[H]
\begin{center}
\includegraphics{/Users/deelim/Documents/Research/Sweave/Project1/BRCA_pam50_CEM.png}
\end{center}
...
\subsubsection{TCGA Pancreatic Cancer Dataset}
