...
%OUR APPROACH%
In this paper, we introduce FSCseq (Feature Selection and Clustering of RNA-seq), which proposes a finite mixture Negative Binomial model (\textbf{NEED BETTER NAME FOR METHOD}), incorporating a combination of the L2 and the SCAD penalty \citep{Fan2001}. By applying this penalty on the difference in log means, we are able to shrink cluster parameters together, while selecting out genes whose difference in expression across clusters is sufficiently low. The adaptive nature of the SCAD penalty reduces the bias introduced by a classic L1 penalty, while maintaining the thresholding aspect of the lasso.
...
\textbf{Not sure if we do this anymore (below)}
