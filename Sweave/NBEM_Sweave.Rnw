...
\hat{\theta}_{kl}^{(m+1)}&=\begin{dcases} 
      sgn(\hat{\theta}_{kl}^{(m)})(\norm{\hat{\theta}_{kl}^{(m+1)}}-\frac{\alpha}{1-\alpha}) & , \norm{\hat{\theta}_{kl}^{(m)}} \leq \frac{2\alpha}{1-\alpha} \\
      \frac{(a-1)\hat{\theta}_{kl}^{(m)} - sgn(\hat{\theta}_{kl}^{(m)})a\frac{\alpha}{1-\alpha}}{a-2} & , \frac{2\alpha}{1-\alpha} < \norm{\hat{\theta}_{kl}^{(m)}} \leq \frac{a\alpha}{1-\alpha} \\
      \hat{\theta}_{kl}^{(m)} & , \norm{\hat{\theta}_{kl}^{(m)}} > \frac{a\alpha}{1-\alpha}
   \end{dcases}
\end{split}
\end{equation}
...
We simulated 24 datasets per condition, and spanned an extensive set of conditions. Variable conditions included log fold change ($LFC$), number of clusters ($K$), and number of samples per cluster ($n_k$). For the purposes of the simulations, we fixed the number of genes ($g$), and the proportion of discriminatory genes ($p_{disc}$) in the dataset. We searched over $LFC=(1, 1.5)$, $K=(1, 2, ..., 7)$, and $n_k=(25, 50)$. We also simulate differences in sequencing depth by taking the size factors estimated from the TCGA Breast Cancer dataset, and incorporating them in our simulations. We then pass them into the FSCseq algorithm as offsets in the IRLS maximization scheme.

To explore the behaviors in various realistic settings, we simulated low and moderate baseline log mean expressions and low, medium, high, and very high noise values based on the initial estimates of expression and dispersion on the TCGA Breast Cancer dataset. These values were $\beta=(3.75,6.5)$ and $\phi=(0.15,0.35,0.50,1.00)$, respectively. We explored simulation cases in the log (not log2) scale in our simulations.

We imposed pre-filtering on the simulated datasets, keeping just the features with the top 20\% of MAD scores. We applied FSCseq on each dataset and assessed the performance statistics under varying levels of baseline expression and noise, and across all combinations of variables ($LFC$, $K$, $n_k$). We measured clustering accuracy by the standard Adjusted Rand Index (ARI), which measures the agreement of derived clusters with the simulated clusters. We measured the feature selection performance by sensitivity (sens), or the proportion of discriminatory genes that were correctly determined to be discriminatory, and false positive rate (FPR), or the proportion of nondiscriminatory genes that were incorrectly determined to be discriminatory. We also analyzed the prediction portion of our algorithm by simulating a new dataset with the same number of features and simulated parameters, but with a smaller sample size $n_{pred}=\floor{0.1n}$. We show the full results in Tables \ref{tab:1} \ref{tab:2}.
...
table1_EM_low=table1[c(1:16,33:48),c(1:13,15)]
table1_EM_hi=table1[c(17:32,49:64),c(1:13,15)]
...
tab3 = xtable(table1_other_low,
              digits=2,
              table.placement="!h",
              caption="Effects of high/low simulated log means ($\\beta = 3.75$ and $6.50$) with low and medium noise ($\\phi = 0.15$ and $0.35$) in simulated datasets on performance, compared to competing methods iCluster+, average-linkage hierarchical clustering (HC) and K-medoids (KM).",
