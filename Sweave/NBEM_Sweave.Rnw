...
table1=table1[,c(1:6,18,7:13,17,14,16,15)]
colnames(table1) = c("$\\beta$","$\\phi$","true K","LFC","$n_{k}$","K","OA","$p_{disc}$","ARI","Sens","FPR","PA","iK","iARI","hcK","hcARI","kmK","kmARI")
tab1 = xtable(table1,
              digits=2,
              table.placement="!h",
              caption="Effects of high/low simulated log means ($\\beta$) and high/low noise ($\\phi$) in simulated datasets on performance. Each row is based on 50 simulated datasets. Order Accuracy (OA) is the proportion of datasets that correctly selected the true K (order), and prediction accuracy (PA) is the proportion of new simulated subjects that were correctly clustered. PA takes into account only runs that correctly selected order, and missing values denote no dataset correctly selected the true K.",
...
tab4 = xtable(tab,digits=3,caption="Cluster Analyses and p-values of TCGA Bladder Cancer data from Log-Rank survival test (L-R) and Fisher's Exact test on NB-EM, iCluster+, Average-linkage hierarchical clustering (HC), and K-medoids (K-Med).",
              label="tab:4",table.placement="H",na.print="",floating.environment = "table*")
print(tab4,sanitize.text.function=identity,table.placement="H")
