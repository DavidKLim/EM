run.sim = function(prefix="",true_k=c(2:7),fold_change=c(0.1,0.3,0.5,0.7,1),num_nondisc=c(10,25,50,75,90)){
  ...
  for(i in 1:length(true_k)){for(j in 1:length(fold_change)){for(k in 1:length(num_nondisc)){
      ...
      out = sprintf("/netscr/deelim/run_sim_%s_%d_%f_%d",prefix,true_k[i],fold_change[j],num_nondisc[k])
...
collect.sim = function(prefix="",true_k=c(2:7),fold_change=c(0.1,0.3,0.5,0.7,1),num_nondisc=c(10,25,50,75,90)){
  ...
  colnames(tab)<-c("true.K","log.fold.change","true.num.disc","K","lambda2","tau","ARI","num.disc","sens","false.pos")
  ii=1
  for(i in 1:length(true_k)){for(j in 1:length(fold_change)){for(k in 1:length(num_nondisc)){
      out = sprintf("/netscr/deelim/run_sim_%s_%d_%f_%d",prefix,true_k[i],fold_change[j],num_nondisc[k])
      out2 = sprintf("%s.out",out)
