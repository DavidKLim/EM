...
fold_change<-0.5               # 50% nondiscriminatory, 50% discriminatory
nondisc_fold_change<-0.1
tt<-1                        # g must be even number (tt is # of discriminatory genes)
...
<<echo=FALSE>>=
print(paste("# of discriminatory genes =",tt))
print(paste("fold change for discriminatory genes:",fold_change))
print(paste('fold change for nondiscriminatory genes:',nondisc_fold_change))
...
<<echo=FALSE>>=
print("lambda1, lambda2, tau, BIC:")
list_BIC[max_index,]
@

<<echo=FALSE,results=hide,warnings=TRUE>>=
...
The results of the final run based on optimal tuning parameters are below:

<<results=tex, echo=FALSE,results=hide>>=
sim=100
temp_pi<-matrix(rep(0,times=max_k*sim),nrow=sim)
temp_coefs<-list()
...
for(ii in 1:sim){
  ...
  X<-EM(y=y,k=max_k,tau=max_tau,lambda1=max_lambda1,lambda2=max_lambda2,size_factors=size_factors)
  temp_pi[ii,]<-X$pi
...
<<echo=FALSE>>=
print(paste("Mean pi:",mean_pi))
print("First 3 genes:")
head(mean_coefs,n=3)
print("Last 3 genes:")
tail(mean_coefs,n=3)
print(paste("Mean % of nondiscriminatory genes:",mean_nondisc))
print(paste("Mean ARI:",mean_ARI))
if(is.na(mean_sensitivity)==FALSE){
  print(paste("Mean sensitivity:",mean_sensitivity))
} else {print("No initially discriminatory genes")}
if(is.na(mean_falsepos)==FALSE){
  print(paste("Mean false positive rate:",mean_falsepos))
} else {print("No initially nondiscriminatory genes")}
@
  \clearpage
